            ...
            { crypto: 'linuxapi' }
                ...
                try {
                    ...
                    if (lyricObj['lrc']) {
                        ...
                        if (version == 1) return;
                        lyricText = lyricText.replace(/\[([a-z]+)\:(.+)\]/gm, "");
                        lyricText = lyricText.replace(/.*作词.*|.*作曲.*|.*終わり.*|.*编曲.*|.*纯音乐.*/gm, "");
                        lyricText = lyricText.replace(/\s\s+/g, " ");
                        lyricText = lyricText.replace(/\[[0-9]{2}\:[0-9]{2}(\.[0-9]{2})?\]([\w:\s]+)\n/gm, "")
                        lyricText = '\[offset\:500\]\n' + lyricText.replace(/\] /g, '\]');
                    }
