...
function _hacks() {
  ...
  this.minimize = () => {
    this.uih.MainWindowState = this.WindowState.Minimized;
  }

  this.normal = () => {
    this.uih.MainWindowState = this.WindowState.Normal;
  }

  this.WindowState = { Normal: 0, Minimized: 1, Maximized: 2 };
...
function _sb(t, x, y, w, h, v, fn) {
  this.paint = (gr, colour) => {
    ...
    if (this.v()) {
      gr.DrawString(this.t, this.font, colour, this.x, this.y, this.w, this.h, SF.CENTRE);
    }
  }
...
function _chrToImg(chr, colour) {
  ...
  temp_gr.DrawString(chr, fontawesome, colour, 0, 0, size, size, SF.CENTRE);
...
function _menu(x, y, flags) {
  ...
  let mm7 = fb.CreateContextMenuManager();
  mm1.Init('File');
  mm2.Init('Edit');
  mm3.Init('View');
  mm4.Init('Playback');
  ...
  fb.IsPlaying && m1.AppendMenuSeparator();
  fb.IsPlaying && s7.AppendTo(m1, MF_STRING, 'Now Playing');
  if (utils.IsKeyPressed(0x10)) {
    m1.AppendMenuSeparator();
    m1.AppendMenuItem(MF_STRING, 901, 'Panel Properties');
    m1.AppendMenuItem(MF_STRING, 902, 'Configure...');
    m1.AppendMenuItem(MF_STRING, 903, 'Reload');
  }
...
function _textHeight(value, font) {
  return _gr.CalcTextHeight(value, font);
}
...
const SF = {
  LT: _StringFormat(0, 0),
  CT: _StringFormat(1, 0),
  RT: _StringFormat(2, 0),
  LC: _StringFormat(0, 1),
  CC: _StringFormat(1, 1),
  RC: _StringFormat(2, 1),
  LB: _StringFormat(0, 2),
  CB: _StringFormat(1, 2),
  RB: _StringFormat(2, 2),
  CENTRE: 285212672
}
...
let tooltip = window.CreateTooltip('Segoe UI', _scale(10));
